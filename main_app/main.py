...
class MyExchange(BaseExchange):
    ...
    def normalize_data(self, data: dict) -> dict[Symbol, TickerInfo]:
        ...
        for ticker in tickers:
            base: str = ticker.get("base", '')
            target: str = ticker.get("target", '')

            symbol = self._convert_symbol_to_ccxt(f"{base}/{target}")
            normalized_data[symbol] = TickerInfo(
                last=float(ticker.get("last", 0)),
                baseVolume=float(ticker.get("volume", 0)),
                quoteVolume=float(ticker.get("converted_volume", {}).get("usd", 0))
            )
        return normalized_data

    async def fetch_tickers(self) -> dict[Symbol, TickerInfo]:

        data = await self.fetch_data(self.base_url + self.postfix + self.params)
